...
staff_long %>%
  ...
  geom_line() +
  labs(
    title = "Faculty Trent over Time (1975-2011)",
    x = "",
    y = ""
  ) +
...
**Answer:**
I would propose making only the Part-time faculty colored on the visualization to highlight its movement above others.
...
**Answer:**
The way I would propose improving the prior visualization is to remove the pie charts and pool each of the countries with more than 1,000,000 tons into their respective continents. Once I accomplish this I would allow free scales for each continent for easier viewing on a histogram graph.
...
```{r}
map_to_continent <- function(country) {
...
fisheries_aggregated <- fisheries_filtered %>%
  group_by(continent = map_to_continent(country)) %>%
  summarise(total_fishing = sum(total))

ggplot(fisheries_aggregated, aes(x = continent, y = total_fishing, fill = continent)) +
  ...
  labs(title = "Fishing by Continent (Countries > 1,000,000 tons)",
       x = "Continent",
       y = "Total Fishing (tons)") +
  theme_minimal() +
...
**Answer:**
My attempt to make the visual easier to read did not work but was a fun attempt. In pooling the countries into continents I did not know how to display easy continent as a separate graph. However, it is interesting to see the prevelance of Asia in comparison to all other continents.
